...
#	Import the Bollywood data set in Rstudio in a variable named bollywood
  ...
  setwd("~/Downloads") #setting the work directory

  bollywood <- read.csv("bollywood.csv")
  View(bollywood) #view the data set of bolywood
...
# Store the movie name in variable named top_2_movie
  ...
  top_2_movie <- bollywood[match(sort(bollywood$Tcollection, decreasing=T)[2], bollywood$Tcollection),"Movie"]
...
# Store the names of 4 movies in same sequence in movie_result vector
  ...
  movie_result <- sapply(bollywood[,4:7], function(x) bollywood[which.max(x),"Movie"])
